...
import logging
import time
from rag_pipeline import rag_pipeline, graph_rag_pipeline
...
app = FastAPI(
    title="Sri Lanka Government Acts Graph RAG API",
    description="API for querying Sri Lankan Government Acts using Graph + RAG",
    version="1.0.0"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # ⚠️ You can restrict this in production
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def root():
    return {
        "message": "Sri Lanka Government Acts Graph RAG API",
        "version": "1.0.0",
        "status": "running"
    }
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # ✅ Use Graph RAG pipeline instead of normal RAG pipeline
        response = graph_rag_pipeline(request.message)
        ...
        result = {
            "response": response,
            "sources": [],  # You can populate this if you return source docs from Neo4j
            "collections": request.collections or [],
            "confidence": 0.85,  # Optional: adjust based on retrieval quality
            "processing_time": processing_time,
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")
        }
