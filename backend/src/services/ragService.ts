...
class RAGService {
  ...
  constructor() {
    ...
    this.timeout = parseInt(process.env.PYTHON_RAG_API_TIMEOUT || '600000', 10);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        logger.info('RAG query completed', {
          processingTime,
          answerLength: (result.answer as string).length,
  ...
  async queryStream(question: string, options: any = {}) {
    const startTime = Date.now();
    ...
    try {
      ...
      const chunks = this.chunkText(response.answer as string, 50);
